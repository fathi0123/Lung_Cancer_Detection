...
namespace Paymentauthorization.Utilities
{
    public static class Buttons
    {
        ...
        {
            return new MvcHtmlString($"<a href='{(string.IsNullOrWhiteSpace(controller) ? urlHelper.Action(action, routeValues) : urlHelper.Action(action, controller, routeValues))}' class='btn btn-soft-primary  waves-effect waves-light px-4'><i class=\"fas fa-plus me-3\"></i> اضافة</a>");
        }
        ...
        {
            return new MvcHtmlString($"<a href='{(string.IsNullOrWhiteSpace(controller) ? urlHelper.Action(action, routeValues) : urlHelper.Action(action, controller, routeValues))}' title='عرض التفاصيل' class='btn btn-soft-primary  waves-effect waves-light p-2'><i class=\"fas fa-eye\"></i></a>");
        }
        ...
        {
            return new MvcHtmlString($"<a href='{(string.IsNullOrWhiteSpace(controller) ? urlHelper.Action(action, routeValues) : urlHelper.Action(action, controller, routeValues))}' title='الرجوع الي القائمة' class='btn btn-soft-info  waves-effect waves-light m-1 col-md-auto px-3'><i class=\"fas fa-arrow-left me-1\"></i>الرجوع الي القائمة</a>");
        }

        public static MvcHtmlString SubmitCreate(this UrlHelper urlHelper)
        {
            return new MvcHtmlString($"<button type='submit' class='btn btn-primary  waves-effect waves-light px-3 m-1 col-12 col-md-auto'><i class='fas fa-plus me-1'></i>إضافة</button>");
        }
